...
北京大学出版社.
\section*{2018-Mar-21}
\noindent 8. 证明$\mathrm{R}^n$中的闭集可表为可列个开集的交, 每个开集可表为可列个闭集的并.
\begin{proof}
...
\noindent 9. ($\mathrm{R}^n$的正规性)设$S_1,S_2$为$\mathrm{R}^n$中不相交的闭集(不一定有界). 证明存在开集$O_1,O_2$满足$S_i\subset O_i,i=1,2$,且$O_1\cap O_2=\emptyset$
\begin{proof}
  取
...
\noindent 10. 设$f_1,\cdots,f_n$是$[0,1]$上的$n$个连续函数,称$f_1,\cdots,f_n$在$[0,1]$上是*线性相关*,若存在不全为零的常数$c_1,\cdots,c_n$,使得
